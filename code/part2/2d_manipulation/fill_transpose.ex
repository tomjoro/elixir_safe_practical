#
# Tanspose to column order and run it
#
# We could chunk every 2 and then just work with lists of 2 bytes.
# Then we could check <<0, 0>> or write whatever it is
# Then we wouldn't need to convert to little16 in both directions
#
#
...
defmodule ReadExample do
  def read_file() do
    ...
    # Contents is a binary and not an Enum
    # We could step through and match with <<:rest>> but it'd be backwards
    # A binary is probably not reversable?

    arr_arr = []
    arr_arr = read_all_rows(contents, width, width, arr_arr) #not 252

    #converted = read_as_array(contents)
    #arr_arr = Enum.chunk(converted, 252)
